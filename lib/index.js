...
/**
 * SCOPIOUS - Customized to use our own messages
 */
const isDebug = process.env.SCOPIOUS_JOI_DEBUG === 'true';
const Joi = internals
    .root()
    .defaults((schema) =>
        schema.prefs({ errors: { wrap: { label: false }, label: 'key' }, abortEarly: false, stripUnknown: true })
    )
    .error((errors) => {
        if (isDebug) {
            console.debug('[ScopiousJoi Debug]', errors);
        }
        return errors;
    })
    .extend(
        ...
        (joi) => ({
            ...
            messages: {
                ...joi.array().messages,
                'array.min': 'at least {{#limit}} {{#label}}(s) are required',
                'array.max': 'must contain at most {{#limit}} {{#label}}(s)',
                'any.required': '{{#label}} is required'
            }
        }),
