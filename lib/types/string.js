...
internals.normalizeEncoding = (enc) => {
    if (typeof enc !== 'string') return undefined;
    const e = enc.toLowerCase().trim();
    if (e === 'utf8') return 'utf-8';
    if (e === 'ucs2' || e === 'ucs-2') return 'utf16le';
    if (e === 'binary') return 'latin1';
    return e;
};

internals.isSupportedEncoding = (enc) => {
    const e = internals.normalizeEncoding(enc);
    if (!e || typeof Buffer === 'undefined' || typeof Buffer.isEncoding !== 'function') return false;
    return Buffer.isEncoding(e) || e === 'base64url';
};

internals.toBase64FromBase64Url = (s) =>
    ...
    rules: {
        ...
        length: {
            ...
            validate(value, helpers, { limit, encoding }, { name, operator, args }) {
                ...
                let length;
                if (encoding) {
                    // normalized in args already, but be defensive
                    const enc = internals.normalizeEncoding(encoding);
                    if (!internals.isSupportedEncoding(enc)) {
                        return helpers.error('string.length', { limit: args.limit, value, encoding }); // preserve prior behavior
                    }
                    if (enc === 'base64url') {
                        // Buffer doesn’t natively accept base64url; normalize to base64
                        length = Buffer.byteLength(internals.toBase64FromBase64Url(value), 'base64');
                    } else {
                        length = Buffer.byteLength(value, enc);
                    }
                } else {
                    length = value.length; // character count when no encoding given
                }
...
internals.length = function (schema, name, limit, operator, encoding) {
    const enc = internals.normalizeEncoding(encoding);
    assert(!enc || internals.isSupportedEncoding(enc), 'Invalid encoding:', encoding);
    return schema.$_addRule({ name, method: 'length', args: { limit, encoding: enc }, operator });
};
